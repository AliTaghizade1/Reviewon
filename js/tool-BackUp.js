...
document.addEventListener('DOMContentLoaded', () => {
    ...
    //     fetch(`get_comments.php?siteId=${SITE_ID}`)
    //         .then(res => res.json())
    //         .then(response => {
    //             if (response.success) {
    //                 allComments = response.data;
    //                 renderSidebar();
    //                 renderPins(); // <--- مهم: رسم پین‌ها بعد از دریافت اطلاعات
    //             }
    //         });
    // }
...
//     commentsList.innerHTML = ''; // پاک کردن لیست
...
//         comment.replies = []; // ایجاد آرایه ریپلای
//     });
...
//         // محاسبه تعداد ریپلای‌ها
//         const replyCount = comment.replies.length;
//         const replyCountHtml = replyCount > 0 ? `<span class="reply-count">${replyCount}</span>` : '';
        
//         let deviceIcon = '💻';
//         if (comment.device_type === 'mobile') deviceIcon = '📱';
//         if (comment.device_type === 'tablet') deviceIcon = '📱';
...
//             ${comment.replies.length > 0 ? `<div class="replies-container">${comment.replies.map(renderReply).join('')}</div>` : ''}
//         `;
//         commentsList.appendChild(item);
//     });

//     // --- Event Delegation (برای دکمه Reply) ---
...
//             // تنظیم فرم برای ریپلای
//             document.getElementById('parentCommentId').value = commentId;
//             document.getElementById('commentText').placeholder = 'Reply...';
//             document.querySelector('#commentForm button[type="submit"]').textContent = 'Reply';
...
function renderSidebar() {
    ...
    // --- Event Delegation (فقط برای چک‌باکس و کلیک روی آیتم) ---
    commentsList.onclick = function(e) {
        const item = e.target.closest('.comment-item');
        if (!item) return;
...
function renderPins() {
    ...
    allComments.forEach(comment => {
        // ✅ تغییر شرط: فقط کامنت‌های اصلی (بدون parent_comment_id) را بررسی کن
        if (comment.device_type === currentDevice && 
            comment.is_resolved === 0 && 
            comment.parent_comment_id === null) { // این خط جدید اضافه شده
            try {
                ...
                if (element) {
                    ...
                    const topPos = comment.offset_y != 0 ? comment.offset_y + 'px' : '-10px';
    ...
    if (currentDevice !== comment.device_type) {
        ...
        if (targetBtn) {
            ...
            return; // خروج از تابع بعد از تغییر دستگاه
        }
    }
    
    // 2. اگر دستگاه یکسان است، اسکرول فوری (بدون تاخیر)
    const iframeDoc = iframe.contentDocument || iframe.contentWindow.document;
    try {
        const element = iframeDoc.querySelector(comment.selector);
        if (element) {
            ...
            <div style="display: flex; gap: 8px;">
                <textarea id="replyInput" placeholder="Write a reply..." 
                        style="flex: 1; padding: 4px; border: 0.5px solid #ccc; border-radius: 4px; 
                                font-family: inherit; font-size: 13px; height: 24px; box-sizing: border-box; resize: none;"></textarea>
    ...
    if (editButton) {
        ...
        //     const commentTextDiv = popup.querySelector('div[style*="white-space: pre-wrap"]');
    ...
    iframe.addEventListener('load', () => {
        ...
        iframeDoc.body.addEventListener('click', (e) => {
            ...
            if (popup && popup.contains(e.target)) {
                return; // اگر روی پاپ‌آپ کلیک شد، کاری نکن
            }
            e.preventDefault();
            
            // محاسبه مختصات موس نسبت به المان کلیک شده
            const rect = e.target.getBoundingClientRect();
            // مختصات موس در صفحه آی‌فریم
            const x = e.clientX; 
            const y = e.clientY;
            
            // محاسبه آفست نسبت به المان
            clickX = Math.round(x - rect.left);
            clickY = Math.round(y - rect.top);

            const selector = getCssSelector(e.target);
            currentSelector = selector; // این خط جدید را اضافه کنید
            currentDevice = document.querySelector('.device-btn.active').dataset.device;
    ...
    commentForm.addEventListener('submit', (e) => {
        e.preventDefault();
        document.getElementById('targetSelector').value = currentSelector;
        document.getElementById('targetDevice').value = currentDevice;
        const formData = new FormData(commentForm);
        formData.append('siteId', SITE_ID);
        formData.append('url', SITE_URL);
        
        formData.append('offsetX', clickX);
        formData.append('offsetY', clickY);
        ...
        .then(data => {
            if (data.success) {
                ...
                loadComments(); // این تابع الان خودش renderPins را صدا می‌زند
            } else {
